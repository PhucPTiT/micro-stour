...
import { CalendarDays, LayoutDashboard, type LucideIcon, PlaneTakeoff } from "lucide-react";
                ...
                <aside className="border-border bg-sidebar/70 border-r px-5 py-6 backdrop-blur">
                    <div className="mb-8 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground text-xs">Shell application</p>
                        </div>
                    </div>
    ...
    return (
        <section className="grid gap-6 lg:grid-cols-[1.25fr_0.95fr]">
            <div className="from-card to-muted/50 rounded-3xl border bg-gradient-to-br p-6">
                <p className="text-muted-foreground mb-2 text-sm font-medium">Shell route</p>
                <h1 className="mb-3 text-3xl font-semibold tracking-tight">
                    Shared layout stays mounted in the shell app.
                </h1>
                <p className="text-muted-foreground max-w-2xl text-sm leading-6">
                    Clicking items in the sidebar only swaps the main content area. Both local and remote routes keep
            ...
            <div className="bg-card rounded-3xl border p-6">
                ...
                <div className="space-y-3 text-sm leading-6">
                    <p>`web` acts as the shell and owns the router plus the shared chrome.</p>
                    <p>Route `/booking` loads a micro app from a separate remote via module federation.</p>
                    <p className="text-muted-foreground">
                        The remote renders only feature content, so it inherits the shell layout automatically.
                    </p>
                </div>
            </div>
        </section>
    ...
    return (
        <Routes>
            <Route element={<ShellLayout />}>
                ...
                <Route element={<ModuleRoute />} path="/module" />
            </Route>
        </Routes>
    );
}
